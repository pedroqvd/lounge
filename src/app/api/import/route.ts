...
Kaique	61993593085
Bernardo	 61998569929 (conferir o número)
...
export async function GET() {
  ...
  for (const line of lines) {
    ...
    if (existingByName) {
      logs.push(\`PULANDO (Nome já existe): \${name}\`)
      skipped++
      continue
    }
    ...
    if (phone) {
      ...
      if (existingByPhone) {
        logs.push(\`PULANDO (Telefone já existe): \${name} - \${phone}\`)
        skipped++
        continue
      }
    }
    ...
    await prisma.member.create({
      data: {
        name,
        phone,
        status: 'VISITANTE', // defaulting to Visitante
      }
    })
    logs.push(\`ADICIONADO: \${name} - \${phone}\`)
    added++
  }

  return NextResponse.json({ added, skipped, logs })
}
