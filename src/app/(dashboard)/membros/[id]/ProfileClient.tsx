...
import { updateMemberNotes, updateMemberMinisterial } from '@/app/actions/crm'
import { User as UserIcon, Calendar, MapPin, Save, MessageCircle, Clock, BookOpen } from 'lucide-react'
...
export default function ProfileClient({ member }: { member: any }) {
  ...
  const handleSaveNotes = async () => {
    ...
    await updateMemberNotes(member.id, notes)
  ...
  const handleSaveMinisterial = async (e: React.FormEvent) => {
    e.preventDefault()
    setIsSavingMin(true)
    await updateMemberMinisterial(member.id, minData)
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
      ...
      <div className="lg:col-span-2 space-y-6">
        ...
        {/* Header Hero */}
        <div className="p-8 bg-card border border-border rounded-3xl shadow-sm relative overflow-hidden">
          ...
          <div className="relative z-10 flex flex-col md:flex-row md:items-end gap-6 pt-12">
            <div className="w-28 h-28 rounded-full bg-background border-4 border-card shadow-lg flex items-center justify-center flex-shrink-0">
      ...
      <div className="p-8 bg-card border border-border rounded-3xl shadow-sm h-full flex flex-col">
        <h2 className="text-xl font-bold mb-6 flex items-center gap-3">
                ...
                <div className="w-[calc(100%-4rem)] md:w-[calc(50%-2.5rem)] p-4 rounded-2xl border border-border bg-background shadow-sm group-hover:shadow-md group-hover:border-primary/30 transition-all">
                  <div className="flex items-center justify-between mb-2">
                    <span className="text-xs font-bold text-muted-foreground uppercase tracking-wider">
                      {new Date(hist.sentAt).toLocaleDateString('pt-BR', {day: '2-digit', month: 'short'})}
                    </span>
